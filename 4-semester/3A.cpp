...
void solve() {
    ...
    vector<vector<size_t>> g(1 + p + m + n + 1);
    const size_t from = g.size() - 2, to = g.size() - 1;
    ...
    for (size_t i = 0; i != n; ++i) {
        ...
        total_profit += price;
        dinica::add_edge(edge, g, p + m + i, to, 0, price);

        for (size_t j = 0; j != k; ++j) {
            size_t tool;
            cin >> tool;
            --tool;
            dinica::add_edge(edge, g, p + tool, p + m + i, 0, INF);
        }
    }
    ...
    for (size_t i = 0; i != p; ++i) {
        ...
        discount[u] = discount[v] = i;
        dinica::add_edge(edge, g, from, i, 0, cost);
    }

    for (size_t i = 0; i != m; ++i) {
        dinica::add_edge(edge, g, (discount.count(i) ? discount[i] : from), p + i, 0, cost[i]);
    }
